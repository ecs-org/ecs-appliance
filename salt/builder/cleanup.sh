...
if test "$PACKER_BUILDER_TYPE" = "googlecompute"; then
    echo "on googlecompute; skip udev,network,openssh-serverkey modifications"
...
echo "Clean up /tmp , /var/tmp"
for a in /tmp /var/tmp; do
    rm -rf $a/.*
    rm -rf $a/*
done
...
if [ -d "/var/lib/dhcp" ]; then
    echo "cleaning up dhcp leases (debian)"
    rm /var/lib/dhcp/*
fi
